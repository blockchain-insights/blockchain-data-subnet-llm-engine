...
def is_chart_applicable(data):
    ...
    funds_flow_applicable = any(
        ('block' in entry or 'block_height' in entry or 'in_total_amount' in entry or 'out_total_amount' in entry or
         ('t1' in entry and ('block' in entry['t1'] or 'block_height' in entry['t1'] or 'in_total_amount' in entry['t1'] or 'out_total_amount' in entry['t1'])))
        for entry in data
    )
...
def convert_balance_tracking_to_chart(data):
    relevant_entries = []
...
def convert_funds_flow_to_chart(data):
    relevant_entries = []

    # Extract relevant data from the structure
    for entry in data:
        if 'block' in entry or 'block_height' in entry or 'in_total_amount' in entry or 'out_total_amount' in entry:
            relevant_entries.append(entry)
        elif 't1' in entry:
            transaction = entry['t1']
            if 'block' in transaction or 'block_height' in transaction or 'in_total_amount' in transaction or 'out_total_amount' in transaction:
    ...
    sorted_entries = sorted(relevant_entries, key=lambda x: x.get('timestamp', x.get('block', x.get('block_height', 0))))

    # Extract data for chart
    labels = [f"Transaction {i+1}" for i in range(len(sorted_entries))]
    blocks = [entry.get('block', entry.get('block_height', None)) for entry in sorted_entries]
